...
const Component = forwardRef<HTMLElement>((props, ref) => {
  const partnerImages = [{
    ...
    src: indiabullsLogo,
    alt: "Indiabulls Group",
    name: "Indiabulls Group"
  }, {
    src: sobhaLogo,
    alt: "Sobha Limited",
    name: "Sobha Limited"
      ...
      <main className='bg-background' ref={ref}>
        <div className='wrapper'>
          <section className='text-foreground h-screen w-full bg-gradient-to-br from-background via-background/95 to-muted grid place-content-center sticky top-0'>
            <div className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,hsl(var(--border))_1px,transparent_1px),linear-gradient(to_bottom,hsl(var(--border))_1px,transparent_1px)] bg-[size:54px_54px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] my-0"></div>
        ...
        <section className='text-foreground w-full bg-background p-4'>
          <div className='grid grid-cols-12 gap-4'>
            <div className='grid gap-4 col-span-4'>
              {partnerImages.slice(0, 3).map((partner, index) => <motion.figure key={`left-${index}`} className='w-full' initial={{
              opacity: 0,
              x: -50
            }} whileInView={{
              opacity: 1,
              ...
              delay: index * 0.2
                  ...
                  <div className="w-full h-96 bg-white/90 rounded-2xl flex items-center justify-center shadow-elegant hover:shadow-luxury hover:scale-105 transition-all duration-300 p-6">
                    <img src={partner.src} alt={partner.alt} className='max-w-full max-h-full object-contain filter grayscale hover:grayscale-0 transition-all duration-300' />
                  </div>
            ...
            <div className='sticky top-0 h-screen w-full col-span-4 gap-4 grid grid-rows-3'>
              {partnerImages.slice(3, 6).map((partner, index) => <motion.figure key={`center-${index}`} className='w-full h-full' initial={{
              opacity: 0,
              scale: 0.8
            }} whileInView={{
              opacity: 1,
            ...
            <div className='grid gap-4 col-span-4'>
              {partnerImages.slice(6).map((partner, index) => <motion.figure key={`right-${index}`} className='w-full' initial={{
              opacity: 0,
              x: 50
            }} whileInView={{
              opacity: 1,
